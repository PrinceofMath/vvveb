...
[data-v-component-sites] [data-v-site-info-*] = $sites['active']['@@__data-v-site-info-([a-zA-Z_]+)__@@']

@site|before = <?php
if(isset($this->sites) && $this->sites[$_sites_idx]) {
	$sites = $this->sites[$_sites_idx];
	
	if (is_array($sites['sites'])) {
		foreach ($sites['sites'] as $index => $site) {?>
		
		@site .dropdown-item|addClass = <?php if ($site['id'] == $sites['site_id']) echo 'active'?>
