...
async function screenshots(dirs = []) {
	...
	*/ 
	...
	for (i in sections) {
		...
		let html = `<html><head><base href="../../"><link href="/css/style.bundle.css" rel="stylesheet"></head><body>${content}</body></html>`;
...
function sections(type = "section", dirs = []) {
	...
	return fs.writeFileSync(/*path.resolve(".") + */`${sectionsDir}/${type}s.js`, sectionsJs);
}
